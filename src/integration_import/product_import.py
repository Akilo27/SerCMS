...
def process_product_csv(xlsx_file, request):
    def process_xlsx(import_xlsx_instance):
        try:
            xlsx_file.seek(0)
            ...
            for row in sheet.iter_rows(min_row=2, values_only=True):
                try:
                    product_data = {
                        field: row[header.index(field)] if field in header else None
                    ...
                    sites = []
                    for site_id in str(product_data["site"]).split(","):
                        try:
                    ...
                    for category_name in str(product_data["category"]).split("|"):
                    ...
                    if product_data["images"]:
                        for image_url in str(product_data["images"]).split(","):
                            try:
                    ...
                    if product_data["atribute"]:
                        for pair in str(product_data["atribute"]).split("|"):
                            if not pair.strip():
                                continue
                            try:
                    ...
                    if product_data["stock"]:
                        for pair in str(product_data["stock"]).split(","):
                            if not pair.strip():
                                continue
                            try:
                                storage_id, quantity = map(int, pair.split(":"))
                ...
                import_xlsx_instance.passed = passed_count
                import_xlsx_instance.added_element = added_count
                import_xlsx_instance.upload_element = updated_count
                import_xlsx_instance.save()

        except Exception as e:
            messages.error(request, f"Ошибка при обработке XLSX: {e}")
            import_xlsx_instance.status = False
            import_xlsx_instance.save()

    import_xlsx_instance = ImportCsv.objects.create(
        type=1,
        status=True,
        quantity=0,
        added_element=0,
    ...
    try:
        ...
        import_xlsx_instance.quantity = row_count
        import_xlsx_instance.save()

        thread = threading.Thread(target=process_xlsx, args=(import_xlsx_instance,))
        thread.start()

    except Exception as e:
        messages.error(request, f"Ошибка при чтении XLSX: {e}")
        import_xlsx_instance.status = False
        import_xlsx_instance.save()
